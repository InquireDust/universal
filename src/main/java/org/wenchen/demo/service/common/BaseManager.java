...
import org.wenchen.demo.util.MpUtil;
...
public class BaseManager<M extends BaseMapper<T>, T> {
    ...
    /**
     * 根据指定Wrapper查询存在的数据数量
     *
     * @param queryWrapper 字段
     */
    public Long countByWrapper(Wrapper<T> queryWrapper) {
     ...
     * 根据对象中的属性值进行删除
     */
    public void deleteByMultFields(Map<SFunction<T, ?>, Object> params) {
        if (params.isEmpty()) {
            return;
        }
        LambdaUpdateChainWrapper<T> lambdaUpdate = lambdaUpdate();
        Iterator<SFunction<T, ?>> iterator = params.keySet().iterator();
        while (iterator.hasNext()) {
            SFunction<T, ?> field = iterator.next();
            Object fieldValue = params.get(field);
            lambdaUpdate = lambdaUpdate.eq(field, fieldValue);
        }
        lambdaUpdate.remove();
    }

    public List<T> findByMultFields(Map<SFunction<T, ?>, Object> params) {
        if (params.isEmpty()) {
            return new ArrayList<>(0);
        }
        ...
        while (iterator.hasNext()) {
            SFunction<T, ?> field = iterator.next();
            Object fieldValue = params.get(field);
            lambdaQuery = lambdaQuery.eq(field, fieldValue);
        }
        return lambdaQuery.list();
    }
}
